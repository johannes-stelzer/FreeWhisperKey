...
struct TranscribeCLI {
    static func main() {
        do {
            ...
            let defaultModel = bundle.defaultModel
            var isDirectory: ObjCBool = false
            guard FileManager.default.fileExists(atPath: defaultModel.path, isDirectory: &isDirectory),
                  !isDirectory.boolValue else {
                throw TranscriptionError.bundleMissing("""
                Default model \(defaultModel.lastPathComponent) is missing at \(defaultModel.deletingLastPathComponent().path).
                Download a ggml model (e.g. by running whisper.cpp/models/download-ggml-model.sh base).
                """)
            }
            let bridge = WhisperBridge(executableURL: bundle.binary, modelURL: defaultModel)
